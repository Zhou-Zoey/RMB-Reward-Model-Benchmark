    ...
    if not custom_dialogue_formatting:
        ...
        def map_conversations(example, core_set=True):
            if core_set:
                ...
                prompt = example["prompt"]
                example["model_1"] = json.loads(prompt) + [{"role": "assistant", "content": example["01-ai/Yi-1.5-6B-Chat"]}]
                example["model_2"] = json.loads(prompt) + [{"role": "assistant", "content": example["internlm/internlm2_5-7b-chat"]}]
                example["model_3"] = json.loads(prompt) + [{"role": "assistant", "content": example["Qwen/Qwen2-7B-Instruct"]}]
                example["model_4"] = json.loads(prompt) + [{"role": "assistant", "content": example["mistralai/Mistral-7B-Instruct-v0.2"]}]
                example["model_5"] = json.loads(prompt) + [{"role": "assistant", "content": example["meta-llama/Meta-Llama-3-8B-Instruct"]}]
            return example
...
def prepare_dialogue(
    ...
    if all(k in example.keys() for k in ("01-ai/Yi-1.5-6B-Chat", "internlm/internlm2_5-7b-chat", "Qwen/Qwen2-7B-Instruct", "mistralai/Mistral-7B-Instruct-v0.2", "meta-llama/Meta-Llama-3-8B-Instruct")):
        # multi turn
        if isinstance(example["prompt"], list) and len(example["prompt"]) > 0:
            # iterate through prompt messages, alternate user and assistant, end with example["chosen"]/rejected
        ...
        # single turn
            ...
            dialogue_template.messages = [
                [dialogue_template.roles[0], example["prompt"]],
                [dialogue_template.roles[1], example["01-ai/Yi-1.5-6B-Chat"]],
            ]
            example["model_1"] = dialogue_template.get_prompt()
            
            dialogue_template.messages = [
                [dialogue_template.roles[0], example["prompt"]],
                [dialogue_template.roles[1], example["internlm/internlm2_5-7b-chat"]],
            ]
            example["model_2"] = dialogue_template.get_prompt()

            dialogue_template.messages = [
                [dialogue_template.roles[0], example["prompt"]],
                [dialogue_template.roles[1], example["Qwen/Qwen2-7B-Instruct"]],
            ]
            example["model_3"] = dialogue_template.get_prompt()

            dialogue_template.messages = [
                [dialogue_template.roles[0], example["prompt"]],
                [dialogue_template.roles[1], example["mistralai/Mistral-7B-Instruct-v0.2"]],
            ]
            example["model_4"] = dialogue_template.get_prompt()

            dialogue_template.messages = [
                [dialogue_template.roles[0], example["prompt"]],
                [dialogue_template.roles[1], example["meta-llama/Meta-Llama-3-8B-Instruct"]],
            ]
